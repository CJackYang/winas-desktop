...
const isHiding = process.env.START_MODE === 'hiding' || process.argv[1] === '--hidden'
...
const initMainWindow = () => {
  ...
  _mainWindow = new BrowserWindow({
    ...
    transparent: false,
    resizable: true,
    title: i18n.__('Default Product Name'),
    center: true,
    icon
  })
...
const onTrayData = (event) => {
  ...
  if (store.getState().login.state === 'LOGIN') {
    ...
    logged = true
    deviceStatus = isCloud ? i18n.__('Connected Via Cloud') : i18n.__('Connected Via LAN')
  ...
  const data = {
    ...
    text: [
      ...
      ['noDrive', i18n.__('No Backup')],
      ['backupDisabled', i18n.__('Backup is Disabled')],
      ['backupFinishedText', i18n.__('Backup Success')],
      ['ChangeDeviceText', i18n.__('Change Device Title')],
      ['UpdateText', i18n.__('Update')],
      ['AboutText', i18n.__('About')],
      ['ExitText', i18n.__('Exit App')]
    ]
  }
...
const onAboutData = (event) => {
  const data = {
    text: [
      ['name', i18n.__('Default Product Name')],
      ['version', i18n.__('Client Version %s', app.getVersion())],
      ['description', i18n.__('App Description')],
      ['copyright', i18n.__('Copyright {{year}}')],
      ['agreement', i18n.__('User Agreement')]
    ]
  }
...
ipcMain.on('SETCONFIG', (event, args) => {
  // set app's autoLaunch
  if (process.platform !== 'linux' && args && args.autoLaunch !== undefined) {
...
ipcMain.on(
  'OPEN_CHANGE_DEVICE',
  () => store.getState().login.state === 'LOGIN' && getMainWindow().webContents.send('CHANGE_DEVICE')
)
ipcMain.on('OPEN_HOME', () => jumpTo('home'))
ipcMain.on('OPEN_ABOUT', openAboutWindow)
ipcMain.on('OPEN_UPDATE', () => shell.openExternal('http://www.aidingnan.com/download'))
ipcMain.on('OPEN_AGREEMENT', () => shell.openExternal('http://www.aidingnan.com'))

ipcMain.on('OPEN_DEVTOOLS', () => _mainWindow.webContents.openDevTools())

module.exports = { initMainWindow, getMainWindow, openTrayWindow }
