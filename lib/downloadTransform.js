...
class Task {
  constructor (props) {
        ...
        const read = async (entries, downloadPath, dirUUID, driveUUID, domain, task) => {
          for (let i = 0; i < entries.length; i++) {
            ...
            if (entry.type === 'directory') {
              ...
              // filter deleted file or dirs
              let children = isPhy ? await serverGetAsync(ep, qs)
                : (await serverGetAsync(ep, qs)).entries.filter(e => !e.deleted && !e.archived)

              // remove the entries with the same name
              const entryMap = new Map()
              children.forEach(e => entryMap.set(e.name, e))
              children = [...entryMap.values()]
    ...
    this.diff = new Transform({
      ...
      push (X) {
        ...
        if (task.isNew) {
          ...
          if (dirEntry.length) {
            this.outs.forEach(t => t.push({ entries: dirEntry, downloadPath, dirUUID, driveUUID, domain, task }))
          }
      ...
      transform: (x, callback) => {
        ...
        const { entries, downloadPath, dirUUID, driveUUID, domain, task } = x
        diffAsync(entries, downloadPath, dirUUID, driveUUID, domain, task)
          .then(y => callback(null, y)).catch(e => callback(e))
      }
    })
    ...
    this.download = new Transform({
      ...
      push (X) {
        ...
        entries.forEach((entry) => {
          if (entry.type === 'directory' || entry.finished) {
            ...
            this.pending.push({ entry, downloadPath, dirUUID, driveUUID, domain, retry: 0, task })
          }
        })
        this.schedule()
      },
      transform: (x, callback) => {
        ...
        debug('task.reqHandles.push(handle)', task.reqHandles.length)
        handle.download()
      }
    })
    ...
    this.readRemote.on('step', () => {
      /* retry, if upload error && retry times < 2 */
      ...
      pipes.forEach((p) => {
        ...
        this[p].failed.forEach((x) => {
          if (Array.isArray(x)) {
            x.forEach((c) => {
              this.errors.push(Object.assign({ pipe: p }, c, { task: c.task.uuid, type: c.entry && c.entry.type }))
            })
  ...
  run () {
    this.paused = false
    this.countSpeed = setInterval(this.countSpeedFunc, 2000)
  ...
  pause (pure) { // if pure === true, don't update store
    debug('pause', this.reqHandles.length)
    this.wait(false)
    ...
    clearInterval(this.countSpeed)
    // update store or prevent store and clear timeout
