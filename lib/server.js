...
const getCloudAddress = () => store.getState().config.cloudAddress || 'https://aws-cn.aidingnan.com/c/v1'
...
ipcMain.on('RefreshToken', (event, { newToken, newCookie }) => {
  console.log('RefreshToken', newToken, newCookie)
  token = newToken
  cookie = newCookie
})
...
const reqCloud = (ep, qsOrData, type, isFormdata) => {
  const url = `${getCloudAddress()}/station/${deviceSN}/json`
  const url2 = `${getCloudAddress()}/station/${deviceSN}/pipe`
  const data = {
    verb: type,
    urlPath: `/${ep}`,
...
const apatch = (ep, data) => {
  if (cloud) return reqCloud(ep, data, 'PATCH')
  const r = request
...
class UploadMultipleFiles {
  ...
  normalUpload () {
    console.log('normalUpload', this.Files.length, this.Files[0].name)
    ...
    this.Files.forEach((file) => {
      ...
      for (let i = 0; i < parts.length; i++) {
        if (policy && policy.seed !== 0 && policy.seed > i) {
          console.log(`skip part ${i} in ${name}`)
          continue // big file, upload from part[seed]
        }
  ...
  upload () {
    try {
      ...
      console.log('uncaught normalUpload error')
      console.log(error)
      this.finish(error)
    }
  }
...
const downloadFile = (entry, downloadPath, callback) => {
  ...
  const dirPath = downloadPath || path.join(getTmpPath(), entryUUID.slice(-12))
...
const updateBackupDrive = (drive, props, cb) => {
  serverGet(`drives/${drive.uuid}`, null, (err, drv) => {
    if (err || !drv) cb(err || 'no drive')
    else {
      const { client } = drv
      Object.assign(client, props)
      apatch(`drives/${drive.uuid}`, { op: 'backup', client }).end((e, r) => {
        const { error, body } = parseRes(e, r)
        cb(error, body)
      })
    }
  })
...
const createBackupDrive = (callback) => {
  serverGet('drives', null, (err, drives) => {
    if (err) return callback(err)
    const { machineId } = global.configuration
    const [hostname, platform] = [os.hostname(), os.platform()]
    const drive = drives.find(d => d.type === 'backup' && d.client && (d.client.id === machineId))
    if (drive) return callback(null, drive)
    const args = {
      op: 'backup',
      label: platform === 'drawin' ? hostname.split('.local')[0] : hostname,
      client: {
        id: machineId,
        status: 'Idle',
        disabled: false,
...
const triggerThumb = (entry, hash, height = 200, width = 200) => {
  const ext = path.parse(entry.toString()).ext.toLowerCase()
  const hasThumb = ['.png', '.jpg', '.jpeg', '.gif', '.bmp', '.mp4', '.mov', '.pdf'].includes(ext)
  if (hasThumb) {
    const ep = `media/${hash}`
    const qs = {
      width,
    ...
    adownload(ep, qs).timeout({
      response: 2000, // Wait 2 seconds for the server to start sending,
      deadline: 2000 // Allow 2 seconds for the file to finish loading.
    }).end(() => {})
    console.log('triggerThumb', entry)
  }
}
