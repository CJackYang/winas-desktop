...
const { serverGetAsync } = require('./server')
...
const readUploadInfoAsync = async (entries, dirUUID, driveUUID, domain) => {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (entryType !== 'others' && (name === sanitize(name)) && (entryType !== 'file' || !isIgnoreFile(name))) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    if (target) {
      ...
      nameSpace.add(checkedName)
      conflicts.push({ entry, entryType, name, checkedName, stat, remote: target })
    }
  }
