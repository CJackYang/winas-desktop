...
ipcMain.on('OPEN_TRANSMISSION', (e, downloadPath) => {
  const dir = path.parse(downloadPath).dir
  fs.access(dir, (err) => {
    console.log('showItemInFolder', downloadPath)
    if (!err) shell.showItemInFolder(downloadPath)
