...
class Task {
  constructor (localEntries, driveId, dirId, onFinished) {
    ...
    this.getData = async () => {
      ...
      let map
      try {
        map = await this.db.readAllAsync()
        this.cache = map
      } catch (error) {
        console.log(error)
        map = new Map()
    ...
    this.readDir = new Transform({
      name: 'readDir',
      isBlocked: () => this.localDiff.pending.length > 16 || this.upload.pending.length > 16,
      concurrency: 2,
    ...
    /* mkdir, diff with local backupDB */
    this.localDiff = new Transform({
      name: 'localDiff',
      concurrency: 2,
      isBlocked: () => this.diff.pending.length > 16,
      transform (x, callback) {
        const read = async (entries, dirUUID, driveUUID, task) => {
          /* read local db */
          const map = await task.getData()
          ...
          for (let i = 0; i < entries.length; i++) {
            ...
            if (stat.isDirectory()) {
              ...
              let uuid
              if (value && value.startsWith('dir')) {
                uuid = value.substr(3)
              }

              if (uuid) {
                console.log('skip dir', fullName)
                /* read child */
                const children = await fs.readdirAsync(path.resolve(entry))
                const newEntries = []
                children.forEach(c => newEntries.push(path.join(entry, c)))
              ...
              try {
                ...
                parts = await hashFileAsync(entry, stat.size, 1024 * 1024 * 1024)
                try {
              ...
              const hash = parts[parts.length - 1].fingerprint
              // local file's key: entry (absolute path)
              // local file's value: type + name + fingerprint + mtime + ctime
              const value = 'file'.concat(fullName).concat(hash).concat(mtime).concat(ctime)
              if (map.get(entry) === value) {
                console.log('skip file', fullName)
                task.completeSize += stat.size
                task.skipSize += stat.size
                task.finishCount += 1
              } else {
                targetEntries.push(entry)
              }
          ...
          console.log(targetEntries)
          return ({ entries: targetEntries, dirUUID, driveUUID, task })
        }
        const { entries, dirUUID, driveUUID, task } = x
        read(entries, dirUUID, driveUUID, task).then(y => callback(null, y)).catch(callback)
      }
    ...
    this.diff = new Transform({
      ...
      transform (x, callback) {
        const read = async (entries, dirUUID, driveUUID, task) => {
          ...
          for (let i = 0; i < entries.length; i++) {
            ...
            if (stat.isDirectory()) {
              ...
              if (uuid) {
                ...
                // local dir's key: absolute path
                // local dir's value: type + uuid
                const value = 'dir'.concat(uuid)
                console.log('task.db.add', entry, value)
                task.db.add(entry, value, err => err && console.err(err))
              }
              task.finishCount += 1
          ...
          // TODO: ignore lack Files or Dirs
          ...
          const readStreams = parts.map((p, i) => {
            ...
            rs.on('end', () => {
              ...
              if (i === parts.length - 1) {
                ...
                const mtime = stat.mtime.getTime()
                const ctime = stat.birthtime.getTime()
                const hash = p.fingerprint
                // local file's key: absolute path
                // local file's value: type + name + fingerprint + mtime + ctime
                const value = 'file'.concat(name).concat(hash).concat(mtime).concat(ctime)
                console.log('task.db.add', entry, value)
                task.db.add(entry, value, err => err && console.err(err))
              }
              sendMsg()
    ...
    this.readDir.pipe(this.localDiff).pipe(this.diff).pipe(this.upload)
