...
class Task {
        ...
        const read = async (changed, dirUUID, driveUUID, rootPath, task) => {
          ...
          if (currentEntries.length) {
            ...
            for (let i = 0; i < entries.length; i++) {
              ...
              task.classifyCount += 1

              if (stat.isFile()) {
                task.classifySize += stat.size
              }
            }
  ...
  status () {
    return Object.assign({}, this.props, {
      ...
      count: Math.max(this.diffCount, this.classifyCount),
      finishCount: this.finishCount,
      size: Math.max(this.diffSize, this.classifySize),
      state: this.state,
