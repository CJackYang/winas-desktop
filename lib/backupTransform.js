  ...
  constructor (localEntries, driveId, dirId, changedTargets, localPath, onFinished) {
    ...
    this.classify = new Transform({
      ...
      transform (x, callback) {
        ...
        const read = async (changed, dirUUID, driveUUID, rootPath, task) => {
          console.log('classify', changed, dirUUID, driveUUID, rootPath)
          ...
          if (deepEntries.length) {
            console.log('deepEntries', deepEntries)
            /* read reomte */
            ...
            deepEntries.forEach((event) => {
              const dirname = event.parts[rootLength + 1]
            ...
            for (let i = keys.length - 1; i >= 0; i--) {
              ...
              try {
                ...
                if (stat.isDirectory) {
                  ...
                  console.log('something changed, ignore this chaneEvent', changeList)
                }
              } catch (e) {
                // dir not found or other error, should handled in parent dir
                console.log('dir not found or  other error, should handled in parent dir', changeList)
                console.error(e)
              }
            }
          }
          ...
          if (currentEntries.length) {
            ...
            const list = await fs.readdirAsync(path.resolve(rootPath))
            const entries = []
    ...
    this.diff = new Transform({
      ...
      transform (x, callback) {
        ...
        console.log('diff', entries.length, 'entries, startsWith:', entries[0])
