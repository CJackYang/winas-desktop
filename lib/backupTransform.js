...
const { readXattrAsync, setXattrAsync, checkBackupFlagAsync, setBackupFlagAsync, clearXattrAsync } = require('./xattr')
const { createBackupDirAsync, UploadMultipleFiles,
  serverGetAsync, updateBackupDirsOrFilesAsync, triggerThumb } = require('./server')
...
const errorLog = (msg) => {
  ...
  getMainWindow().webContents.send(
    'UMENG_LOG',
    { category: 'btransform', action: 'error', msg }
  )
}
...
class Task {
  constructor (localEntries, driveId, dirId, changedTargets, localPath, onFinished) {
    ...
    // fast read entire dir, it will be called on backup turn first started
    this.readDir = new Transform({
      ...
      transform (x, callback) {
        ...
        const read = async (entries, tree, task) => {
          // treat first level dirs as changed
          let trueChanged = !!tree.root
          let dirCount = 0
          let dirSize = 0
          for (let i = 0; i < entries.length; i++) {
            ...
            if (stat.isDirectory()) {
              ...
              // treat first level dirs as changed
              if (!res.trueChanged && backuped && !tree.root) {
          ...
          // debug('readDir trueChanged', trueChanged, entries.length > 1 && entries[0])
    ...
    // fast count changed file's number and size, it will be called on changed fired
    this.fastCount = new Transform({
      ...
      transform (x, callback) {
        ...
        const read = async (changed, dirUUID, driveUUID, rootPath, task) => {
          ...
          // treat first level files or dirs as changed
          let trueChanged = !!changed.root
    ...
    // get dirs need further diff from `changed`
    ...
    // diff local items with remote, mkdir if need
    this.diff = new Transform({
      ...
      transform (x, callback) {
        const read = async (entries, dirUUID, driveUUID, task) => {
          debug('diff', entries, 'in', dirUUID, driveUUID)
          /* read local */
          const localFiles = []
          const localDirs = []
          ...
          const lackDirs = remoteDirs.filter(rd => !rd.deleted && !localDirs.find(d => (d.name === rd.bname)))

          debug('remoteFiles', remoteFiles.length)
          debug('localFiles', localFiles.length)
          debug('newFiles', newFiles)
          debug('diffFiles', diffFiles)
          debug('lackFiles', lackFiles)
          debug('lackDirs', lackDirs)
    ...
    this.upload = new Transform({
      ...
      transform: (X, callback) => {
        ...
        const handle = new UploadMultipleFiles(driveUUID, dirUUID, Files, domain, (error) => {
          ...
          // ignore upload error when upload file to deleted directory
          if (error && error.status === 400 && error.response &&
            error.response.message === 'invaild op for deleted dir') {
            console.warn('ignore upload error when upload file to deleted directory')
            newError = null
          }
          ...
          if (error) {
            ...
            // clearXattrAsync
            Files.forEach(f => clearXattrAsync(f.entry).catch(console.error))
          }

          if (newError) errorLog(`backup upload failed: ${newError.code}`)
          if (!error) {
            Files.forEach((f) => {
          ...
          callback(newError, { driveUUID, dirUUID, Files, task, domain })
        })
    ...
    this.onStep = () => {
      ...
      if (this.root.isStopped() || this.errors.length > 4) this.finish()
    }
