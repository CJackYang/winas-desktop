...
class Task {
  constructor (localEntries, driveId, dirId, changedTargets, localPath, onFinished) {
      ...
      transform (x, callback) {
        ...
        const read = async (entries, tree, task) => {
          for (let i = 0; i < entries.length; i++) {
            ...
            if (stat.isDirectory()) {
              /* read child */
              tree.children.push({ value: fullName, children: [] })
              const children = await fs.readdirAsync(path.resolve(entry))
              const newEntries = []
              children.forEach(c => newEntries.push(path.join(entry, c)))

              read(newEntries, tree.children[tree.children.length - 1], task)
            } else {
              if (!backuped) tree.children.push({ value: fullName, children: [] })
              task.size += stat.size
            }
          }

          return ({ changed: tree, entries, dirUUID: dirId, driveUUID: driveId, rootPath: localPath, task })
        }
    ...
    this.classify = new Transform({
      ...
      transform (x, callback) {
        ...
        const read = async (changed, dirUUID, driveUUID, rootPath, task) => {
          debug('classify', changed.length, dirUUID, driveUUID, rootPath)
    ...
    this.diff = new Transform({
      ...
      transform (x, callback) {
        const read = async (entries, dirUUID, driveUUID, task) => {
          debug('diff', entries.length, 'in', dirUUID, driveUUID)
          /* read local */
          const localFiles = []
          const localDirs = []
    ...
    this.upload = new Transform({
      ...
      transform: (X, callback) => {
        debug('upload X', X.files[0] && X.files[0].entry)
