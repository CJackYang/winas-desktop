...
class Configuration {
  constructor (root) {
    this.root = path.join(root, 'pocket_drive')
  }
  ...
  */
  ...
  getHostname () {
    return os.platform() === 'darwin' ? os.hostname().split('.local')[0] : os.hostname()
  }
  ...
  getWisnucDownloadsDir () {
    return path.join(app.getPath('downloads'), 'pocket_drive')
  }
  ...
  */
  ...
  async loadObjectAsync (fpath) {
    ...
    try { obj = JSON.parse(await fs.readFileAsync(fpath)) } catch (e) {
      console.error('loadObjectAsync error', e)
      return null
    }
  ...
  async updateGlobalConfigAsync (newConfig) {
    ...
    global.dispatch({
      ...
      data: {
        ...
        cloudAddress: this.globalConfig.getConfig().cloudAddress,
  ...
  async initAsync () {
    ...
    for (let i = 0; i < UUIDs.length; i++) {
      try { userConfigs.push(await this.initUserConfigAsync(UUIDs[i])) } catch (e) { console.error('init config error', e) }
    }
    ...
    mkdirpAsync(downloadPath).catch(console.error)

    /* get machineId */
    this.machineId = (await machineId()).slice(-8)
