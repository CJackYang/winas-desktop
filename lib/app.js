...
const updateTray = () => {
  ...
  if (!tray) {
    tray = new Tray(icon)
    tray.setToolTip(i18n.__('Default Product Name'))
...
app.on('ready', () => {
  ...
  configuration.initAsync()
    .then(() => {
      ...
      const defaultLocale = (config.global && config.global.locales)
        ? config.global.locales : (/zh/.test(app.getLocale()) ? 'zh-CN' : 'en-US')
