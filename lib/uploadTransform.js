...
const { isIgnoreFile } = require('./utils')
...
const { createFoldAsync, UploadMultipleFiles, serverGetAsync, triggerThumb } = require('./server')
...
class Task {
  constructor (props) {
    ...
    this.countSpeedFunc = () => {
      ...
      const dl = this.deltaSizeList
      // last ten seconds' uploading size
      dl.unshift(this.completeSize - this.skipSize)
      if (dl.length > 10) {
        dl.pop()
      }
      this.speed = Math.max((dl[0] - dl[dl.length - 1]) / (dl.length - 1), 0)
    ...
    this.readDir = new Transform({
      name: 'readDir',
      isBlocked: () => this.mkdir.pending.length > 16,
      concurrency: 2,
          ...
          for (let i = 0; i < entries.length; i++) {
            ...
            try {
              ...
              if (fullName !== sanitize(fullName)) {
                task.warnings.push({
                  pipe: 'readDir', entry, error: { code: 'ENAME' }, stat, task: task.uuid, type: 'unknown'
                })
                console.error('invalid name and lstat error', entry)
                continue
              } else {
            ...
            /* filter skip files */
            if (stat.isFile() && isIgnoreFile(fullName)) {
              task.warnings.push(Object.assign({
                pipe: 'readDir', entry, error: { code: 'ESKIP' }, stat, task: task.uuid, type
              }))
    ...
    this.mkdir = new Transform({
      name: 'mkdir',
      isBlocked: () => this.hash.pending.length > 16,
      concurrency: 2,
      transform (x, callback) {
        const read = async (entries, dirUUID, driveUUID, policies, domain, task) => {
          const files = []
          for (let i = 0; i < entries.length; i++) {
            ...
            /* filter skip files */
            if (stat.isFile() && isIgnoreFile(fullName)) continue
    ...
    this.hash = new Transform({
      ...
      push (x) {
        ...
        if (!files.length) {
          /* task all finished */
          if (!task.paused && task.finishCount === task.count &&
            task.readDir.isSelfStopped() && task.diff.isStopped() && !this.pending.length) {
            task.finish()
          }
        } else {
          files.forEach((f) => {
        ...
        const diffAsync = async (local, driveUUID, dirUUID, domain, task) => {
          ...
          remote.forEach((r) => {
            ...
            if (map.has(rKey)) {
              ...
              task.skipSize += map.get(rKey).stat.size
              debug('skip file:', r.name)
              map.delete(rKey)
            }
          ...
          if (mapValue.length) {
            ...
            mapValue.forEach((l) => {
              ...
              if (l.parts.length > 0) {
                ...
                if (index > 0) {
                  seed = index
                  task.completeSize += index * 1024 ** 3
                  task.skipSize += index * 1024 ** 3
                  debug('skip part of file:', checkedName, index + 1, 'G')
                }
              }
    ...
    this.upload = new Transform({
      ...
      push (X) {
        ...
        X.forEach((x) => {
          if (x.type === 'directory') {
            ...
            const MAX = 2
            const i = this.pending.findIndex(p => p.length < MAX &&
      ...
      transform: (X, callback) => {
        ...
        try {
          Files = X.map((x) => {
            const { entry, stat, parts, policy, retry } = x
            if (retry > 0) {
              debug('retry:', retry, 'entry:', entry)
            }
            ...
            const readStreams = parts.map((p, i) => {
              ...
              rs.on('error', error => callback(error))
          ...
          // create readStreams failed, maybe source file deleted
          debug('create readStreams failed')
          X.forEach((x) => {
            if (x.retry > -1) x.retry += 1
            else x.retry = 0

            // handle skip size
            if (x.policy && x.policy.seed) {
              task.completeSize -= x.policy.seed * 1024 ** 3
            }
          })
          callback(error)
          return
        ...
        handle = new UploadMultipleFiles(driveUUID, dirUUID, Files, domain, (error) => {
          ...
          if (error) {
            ...
            debug('countSum && uploadedSum', countSum, uploadedSum, task.finishCount, task.completeSize)
            X.forEach((x) => {
              if (x.retry > -1) x.retry += 1
              else x.retry = 0
            ...
            Files.forEach((f) => {
              const hash = f.parts[f.parts.length - 1].fingerprint
              triggerThumb(f.entry, hash)
            })
          }
          callback(error, { driveUUID, dirUUID, Files, task, domain })
        })

        // push to reqHandles, need for aborting task
    ...
    this.readDir.on('step', () => {
      ...
      for (let i = this.upload.failed.length - 1; i > -1; i--) {
        ...
        if (index > -1) {
          console.log('X retry', X[0].retry, X[0].error)
          const files = []
  ...
  status () {
    return Object.assign({}, this.props, {
      ...
      targetsCount: this.entries.length
    })
  }
  ...
  updateStore () {
    if (!this.WIP && !this.storeUpdated) {
      ...
      this.countStore += 1
      this.storeTimer = setTimeout(() => this && !(this.WIP = false) && this.updateStore(), 100)
  ...
  pause (pure) { // if pure === true, don't update store
    ...
    else clearTimeout(this.storeTimer)
    sendMsg()
  }
  ...
  finish () {
    ...
    if (this.entries.length > 1) name = name.concat(i18n.__n('And Other %s Items', this.entries.length))
