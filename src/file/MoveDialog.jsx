...
class MoveDialog extends React.PureComponent {
  constructor (props) {
    ...
    this.back = () => {
      ...
      if (['directory', 'public', 'built-in', 'home', 'share'].includes(currentDir.type)) { // normal directory
        this.list(driveUUID, dirUUID)
    ...
    this.move = () => {
      ...
      const entries = this.selectedArr.map(e => e.name)
      ...
      this.props.apis.pureRequest('copy', { type, src, dst, entries, policies }, this.finish)
    }
    ...
    this.finish = (error, data) => {
      ...
      if (error) {
        ...
        this.getTaskState(data.uuid).then((res) => {
          ...
          if (res !== 'Finished') this.props.showTasks()
        }).catch((e) => {
          this.setState({ loading: false })
          this.closeDialog()
          this.props.refresh()
